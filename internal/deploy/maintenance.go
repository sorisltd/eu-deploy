...
func renderProxyReloadCommands(opts RemoteOptions, rootCaddyPath, proxySitesDir, proxyDataPath, proxyConfigPath string) []string {
	serverRootPath := strings.TrimSpace(opts.RemoteServerPath)
	if serverRootPath == "" {
		serverRootPath = "/opt/eu-deploy"
	}

	return []string{
		fmt.Sprintf("if docker ps --format '{{.Names}}' | grep -Fx -- %s >/dev/null 2>&1; then", shellQuote(opts.ProxyContainerName)),
		fmt.Sprintf("  if docker inspect %s --format '{{range .Mounts}}{{println .Source \"->\" .Destination}}{{end}}' | grep -Fx -- %s >/dev/null 2>&1; then",
			shellQuote(opts.ProxyContainerName),
			shellQuote(serverRootPath+" -> "+serverRootPath)),
		...
		fmt.Sprintf("    docker run -d --restart unless-stopped --network host --name %s -v %s:/etc/caddy/Caddyfile:ro -v %s:/etc/caddy/sites:ro -v %s:/data -v %s:/config -v %s:/var/log/caddy -v %s:%s:ro caddy:2 >/dev/null",
			shellQuote(opts.ProxyContainerName),
			...
			shellQuote(serverRootPath),
			shellQuote(serverRootPath)),
		"  fi",
		"elif CADDY_CONTAINER=$(docker ps --filter 'ancestor=caddy:2' --format '{{.Names}}' | head -1) && [ -n \"$CADDY_CONTAINER\" ]; then",
		fmt.Sprintf("  if docker inspect \"$CADDY_CONTAINER\" --format '{{range .Mounts}}{{println .Source \"->\" .Destination}}{{end}}' | grep -Fx -- %s >/dev/null 2>&1; then",
			shellQuote(serverRootPath+" -> "+serverRootPath)),
